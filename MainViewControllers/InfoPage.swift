...
//            wavesView = UIImageView(image: UIImage.init(named: "waves", in: OTRAssets.resourcesBundle, with: nil))
        } else {
        ...
        UIView.animate(withDuration: 1.2, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 1, options: .curveEaseOut, animations: {
            self.wavesView!.frame.origin.y = self.view.frame.origin.y+50
        })
    }
